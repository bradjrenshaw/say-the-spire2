...
public class DailyRunGameScreen : GameScreen
{
    private static readonly FieldInfo? LobbyField =
        AccessTools.Field(typeof(NDailyRunScreen), "_lobby");

    public static DailyRunGameScreen? Current { get; private set; }
    ...
    private RichTextLabel? _dateLabel;
    private RichTextLabel? _timeLeftLabel;
    ...
    public DailyRunGameScreen(NDailyRunScreen screen)
    {
        _screen = screen;
        RootElement = _root;
    ...
    public void OnPlayerChanged(LobbyPlayer player)
    {
        ...
        var character = player.character?.Title?.GetFormattedText() ?? Ui("DAILY_RUN.NO_CHARACTER");
        var ready = player.isReady ? Ui("DAILY_RUN.READY") : Ui("DAILY_RUN.NOT_READY");
    ...
    protected override void BuildRegistry()
    {
        ...
        RegisterFocusable(_dateLabel, new ActionElement(
            () => GetDateText() ?? Ui("DAILY_RUN.FALLBACK"),
            status: () => GetTimeLeftText()));
        ...
        RegisterFocusable(_unreadyButton, new ActionElement(
            ...
            typeKey: () => "button",
            isVisible: () => IsMultiplayer() && IsVisible(_unreadyButton)));
    ...
    private void ResolveControls()
    {
        ...
        _characterContainer ??= _screen.GetNodeOrNull<NDailyRunCharacterContainer>("ChallengeContainer/CenterContainer/HBoxContainer/CharacterContainer");
        _dateLabel ??= _screen.GetNodeOrNull<RichTextLabel>("%Date");
        _timeLeftLabel ??= _screen.GetNodeOrNull<RichTextLabel>("%TimeLeft");
    ...
    private StartRunLobby? Lobby => LobbyField?.GetValue(_screen) as StartRunLobby;
    ...
    private string GetCharacterLabel()
    {
        ...
        return character?.Title?.GetFormattedText() ?? Ui("DAILY_RUN.CHARACTER");
    }
    ...
    private string? GetCharacterStatus()
    {
        ...
        if (IsMultiplayer())
            parts.Add(GetPlayerName(lobby.LocalPlayer.id));
    ...
    {
        return _dateLabel == null ? null : ProxyElement.StripBbcode(_dateLabel.Text);
    }

    private string? GetTimeLeftText()
    {
        return _timeLeftLabel == null ? null : ProxyElement.StripBbcode(_timeLeftLabel.Text);
    }
    ...
    {
        return IsMultiplayer() ? Ui("DAILY_RUN.READY") : Ui("DAILY_RUN.START_DAILY_RUN");
    }
